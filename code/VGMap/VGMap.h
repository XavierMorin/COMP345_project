...
public:
	...
	BuildingSlot(std::vector<BuildingSlot *> adjacentBuildings_, int Points_); //parameterized constructor with adjacent BuildingSlots
	~BuildingSlot(); // Destructor
	

	// Accessors 
	int& getPoints();    // Points accessor
	int& getBuilding();  // Building accessor 
	...
	void addAdjacentBuildingSlot(BuildingSlot *BuildingSlot);
...
private:
	...
	std::string *BuildingBuilt; // to be changed to object Building
